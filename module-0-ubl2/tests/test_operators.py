...
def test_sigmoid(a):
    ...
    * Check properties of the sigmoid function, specifically
    ...
    assert sigmoid(-10) < sigmoid(-1) < sigmoid(0) < sigmoid(9) < sigmoid(10)
...
def test_other():
    ...
    """
    Any number multiplied with 1.0 return the same number.
    Any number added with zero return the same number
    """
    assert add(a, 0) == a
    assert mul(a, 1) == a
...
def test_sum_distribute(ls1, ls2):
    """
    Write a test that ensures that the sum of combined list is equal to sum of individual elements in the list.
    """
