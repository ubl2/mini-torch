...
def topological_sort(variable, deriv):
    ...
    derivatives_queue.put((variable, deriv))

    while not derivatives_queue.empty():
        (varia, deri) = derivatives_queue.get()
        result.append((varia, deri))
        if varia.is_leaf():
            continue
        else:
            backprop_result = varia.history.backprop_step(deri)
            for v in backprop_result:
                derivatives_queue.put(v)
...
def backpropagate(variable, deriv):
    ...
    """
    topo_list = topological_sort(variable, deriv)
    for (varia, deri) in topo_list:
        if varia.is_leaf():
            varia.accumulate_derivative(deri)
    ...
    #         varia.accumulate_derivative(deri)
    #     else:
    #         backprop_result = varia.history.backprop_step(deri)#able_with_deriv.variable.history.backprop_step(variable_with_deriv.deriv)
    #         for v in backprop_result:
    #             derivatives_queue.put(v)
